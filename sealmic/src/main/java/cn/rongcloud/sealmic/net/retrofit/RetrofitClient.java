...
public class RetrofitClient {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!originalResponse.headers("Set-Cookie").isEmpty()) {
                ...
                SharedPreferences.Editor config = mContext.getSharedPreferences(NetConstant.SP_NAME_NET, MODE_PRIVATE)
                        .edit();
                config.putStringSet(NetConstant.SP_KEY_NET_COOKIE_SET, cookiesSet);
                config.commit();
            }
    ...
    public class AddHeaderInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            SharedPreferences preferences = mContext.getSharedPreferences(NetConstant.SP_NAME_NET,
                    Context.MODE_PRIVATE);

            //添加cookie
            HashSet<String> cookieSet = (HashSet<String>) preferences.getStringSet(NetConstant.SP_KEY_NET_COOKIE_SET, null);
            if (cookieSet != null) {
            ...
            //添加用户登录认证
            String auth = preferences.getString(NetConstant.SP_KEY_NET_HEADER_AUTH, null);
            if(auth != null) {
